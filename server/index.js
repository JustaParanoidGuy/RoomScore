...
async function readDb() {
  try {
    const raw = await fs2.readFile(DATA_FILE, "utf8");
...
async function writeDb(db) {
  await fs2.writeFile(DATA_FILE, JSON.stringify(db, null, 2), "utf8");
}

async function readUsers() {
  try {
    const raw = await fs2.readFile(USERS_FILE, "utf8");
    const j = JSON.parse(raw);
...
async function writeUsers(db) {
  await fs2.writeFile(USERS_FILE, JSON.stringify(db, null, 2), "utf8");
}
...
app.get("/api/posts", async (_req, res) => {
  try {
    ...
    res.json({ posts: db.posts.map(withImageUrl) });
...
app.get("/api/export", async (_req, res) => {
  try {
    ...
    for (const p of db.posts) {
      ...
      const copy = { ...rest };
      if (imageFile) {
        const buf = await fs2.readFile(path.join(UPLOADS, imageFile));
        const ext = path.extname(imageFile).replace(".", "") || "jpeg";
        const mime = ext === "jpg" ? "jpeg" : ext;
        copy.imageDataUrl = "data:image/" + mime + ";base64," + buf.toString("base64");
      }
      posts.push(copy);
    }
...
app.patch("/api/posts/:id/like", async (req, res) => {
  try {
    ...
    p.likes = Math.max(0, (p.likes || 0) + d);
    await writeDb(db);
...
app.delete("/api/posts/:id/comments/:cid", authMiddleware, async (req, res) => {
  try {
    ...
    if (p.comments[ci].author !== req.user.username)
...
app.post("/api/posts/import", express.json({ limit: "80mb" }), async (req, res) => {
  try {
    ...
    for (const raw of posts) {
      ...
      if (idSeen.has(id)) id = nanoid();
      idSeen.add(id);
      const post = {
        id, title: String(raw.title || "Untitled").slice(0, 60),
        room: String(raw.room || "Other"), style: String(raw.style || "Other"),
        description: String(raw.description || "").slice(0, 400),
        tags, imageFile, w: Number(raw.w) || 0, h: Number(raw.h) || 0,
    ...
    await writeDb(db);
    res.json({ ok: true, count: db.posts.length });
